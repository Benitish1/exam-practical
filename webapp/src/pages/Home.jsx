  ...
  return (
    <main className="flex flex-col w-full h-full justify-center items-center overflow-y-auto">
      ...
      <div className="mt-4">
        <button
          type="button"
          className="px-4 py-2 bg-green-400 text-gray-950 rounded-md"
      ...
      <div className="flex flex-col gap-3 mt-5 w-full md:w-[500px] px-3 mb-9 ">
        {tasks.map((task, idx) => (
          ...
          >
            <div className="flex w-full justify-between">
              ...
              <div className="flex gap-4">
                <Link to={`/edit/${task._id}`}>
                  <PenIcon className="h-5 w-5 text-yellow-300" />
                </Link>
            ...
            <p className="font-light text-gray-300">{task.description}</p>
          </div>
        ))}
      </div>
      {isOpen ? (
